...
Previously, our model explained 0.5390 of the variance, and with regularization we see an improvement in both cases. Also, the values for RMSE improved, which means our model's performance improved after applying regularization.
...
# rpart.plot(dt_model, main = "Decision Tree for the Diagnosis of Breast Cancer")
##############################################
...
# Make predictions 
predictions_log_reg <- predict(logistic_model, test_data[-1], type = "response")

# Convert probabilities into diagnosis labels
predicted_classes_log_reg <- ifelse(predictions_log_reg > 0.5, "M", "B")
predicted_classes_log_reg <- factor(predicted_classes_log_reg, levels = levels(test_data$diagnosis))

# Evaluate the model
confusion_matrix_log_reg <- confusionMatrix(predicted_classes_log_reg, test_data$diagnosis)
print(confusion_matrix_log_reg)

# Generate ROC Curve for Decision Tree
#roc_curve_log_reg <- roc(test_data$diagnosis, predicted_classes_log_reg, levels = levels(test_data$diagnosis), direction = "auto")

roc_curve_log_reg <- roc(as.numeric(test_data$diagnosis)-1, as.numeric(predictions_log_reg)-1)

plot(roc_curve_log_reg, col = "#036169", main = "ROC Curve for the Logistic Regression model")
auc(roc_curve_log_reg) # The area under the ROC curve is 0.9903
...
```{r}
# #setwd("C:/Users/arian/Documents/GitHub/assignment-3-Arimurilloh-main")
# #Load libraries
# library(glmnet)
# library(pROC)
# library(class)
...
# 
# ```{r}
# library(dplyr)  # To use full_join and mutate
# 
# # Define the base folder for LAML
# base_path_LAML <- "data/LAML100"
# 
# # Obtain the list of .tsv files inside subfolderss
...
# for (i in seq_along(LAML_files)) {
#   file <- LAML_files[i]  # # Iterate over the files and process them
# 
#   # Merge with the accumulating dataframe
#   if (file.exists(file)) {
#     df <- read.table(file, header = TRUE, skip = 1, sep = "\t")
#     
#     # Ensure the dataframe has enough rows before removing them
#     if (nrow(df) > 4) {
#       df <- df[-c(1:4), ]  # Remove the first 4 rows
#     }
# 
#     # Select only the columns of interest if they exist
...
#         LAML <- df_subset
#       } else {
#         LAML <- full_join(LAML, df_subset, by = "gene_id")
#       }
#     } else {
#       warning(paste("El archivo", file, "no tiene las columnas esperadas."))
#     }
#   } else {
#     warning(paste("Archivo no encontrado:", file))
#   }
# }
# 
...
# colnames(LAML) <- LAML[1,]  # Set column names as gene_ids
# LAML <- LAML[-1,]  # Remove the first row
# rownames(LAML) <- paste0("LAML_", 1:nrow(LAML))  # Dynamically rename rows
# 
# # Define the base folder for KIRC
# base_path_KIRC <- "data/KIRC100"
# 
# # Get the list of .tsv files inside subfolders
# KIRC_files <- list.files(base_path_KIRC, pattern = "\\.tsv$", recursive = TRUE, full.names = TRUE)
# 
# # Get the list of .tsv files inside subfolders
# KIRC <- data.frame()
# 
# # Iterate over the files and process them
# for (i in seq_along(KIRC_files)) {
#   file <- KIRC_files[i]  # Each file already has the full path
# 
...
#         KIRC <- df_subset
#       } else {
#         KIRC <- full_join(KIRC, df_subset, by = "gene_id")
#       }
#     } else {
#       warning(paste("El archivo", file, "no tiene las columnas esperadas."))
#     }
...
# 
# # Set column and row names
# colnames(KIRC) <- KIRC[1,]  # Set column names as gene_ids
# KIRC <- KIRC[-1,]  # Remove the first row
# rownames(KIRC) <- paste0("KIRC_", 1:nrow(KIRC))  # Dynamically rename rows
# 
# # Merge the datasets
# merge_data <- rbind(LAML, KIRC)
# merge_data <- merge_data %>% mutate(type = ifelse(grepl("^LAML", rownames(merge_data)), "LAML", "KIRC"))
# 
# # Load the unknown data
# unknown_data_path <- "data/unknwown_samples.tsv"
# 
...
# } else {
#   warning(paste("El archivo de datos desconocidos no fue encontrado:", unknown_data_path))
# }
# 
...
# KIRC <- lapply(KIRC, as.numeric)
# LAML <- as.data.frame(LAML)
# KIRC <- as.data.frame(KIRC)
# 
# # Calculate the absolute difference in gene expression between cancer types
# expression_difference <- abs(apply(LAML, 2, mean) - apply(KIRC, 2, mean))
# 
# # Sort genes by expression difference
# sorted_genes <- sort(expression_difference, decreasing = TRUE)
# 
# # Display descriptive statistics
# range(expression_difference)
# mean(expression_difference)
# median(expression_difference)
# 
# # Select the top 50 genes with the highest expression difference
# genes_dif <- head(sorted_genes, 50)
# genes_dif <- as.data.frame(genes_dif)
# top50genes <- rownames(genes_dif)
# 
# # Filter the dataset, keeping only the selected genes and the type variable
# filtered <- merge_data[, c(top50genes, "type"), drop = FALSE]
# 
# # Training and test split
# set.seed(2024)  # For reproducibility
# training_indices <- sample(1:nrow(filtered), 0.7 * nrow(filtered))  # 70% of data for trainingo
# test_indices <- setdiff(1:nrow(filtered), training_indices) # 30% for testing
# 
# train_data <- filtered[training_indices, ]
# test_data <- filtered[test_indices, ]
# 
# # Convert genes to numeric and type variable to factor
...
# 
# # Create workflow and cross-validation scheme
# rf_workflow <- workflow() %>% 
#   add_recipe(cancer_recipe) %>% 
#   add_model(cancer_model)
# 
# training_cv <- vfold_cv(train_data) # 5 folds by default
# 
# # Perform hyperparameter tuning
# set.seed(2024)
# cancer_tune <- tune_grid(
#   rf_workflow,
...
# 
# # ROC CURVE ----------------------------------------------------------------------
# # Convert predictions to numeric for ROC
...
# 
# # Plot the ROC curve with customized colors
# plot(cancer_roc, 
...
#      print.auc = TRUE,
#      auc.polygon = TRUE, 
#      auc.polygon.col = rgb(1, 0, 0, alpha = 0.2))  # Transparent red shading
# grid(col = "gray70")  # Add a light grid
# 
# ```
...
# ```{r}
# library(dplyr)      # Data manipulation
# library(tidymodels) # Model handling
# library(ggplot2)    # Visualization
# 
# # PREDICTIONS OF THE UNKNOWN SAMPLES ---------------------------------------------------
# 
# # Ensure the unknown dataset contains the same top 50 genes
...
# 
# # Get the class with the highest probability for each individual
...
#   Cancer_Class_Prediction = gsub('.pred_', '', predicted_class),  # Remove prefix
...
#   theme_minimal() +
#   scale_fill_manual(values = c("#1f78b4", "#e31a1c"))  # Blue & Red for readability
# 
# # Set probability threshold (adjust as needed)
# threshold <- 0.9  # Example: Only consider predictions with >90% confidence
# 
# # Count number of observations above the threshold
...
```
We have 6 observations with a probability higher than 0.9. 
