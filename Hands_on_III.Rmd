...
# Look at the distribution of the target variable: diagnosis
num_counts <- as.data.frame(table(data_breast_cancer$diagnosis)) # There are 357 benign samples and 212 malign samples
```
Now that we have explored the data, we proceed with the KNN algorithm
```{r}
## KNN algorithm
...
# Normalize data: this step is crucial for the KNN algorithm
...
plot(roc_curve_knn, col = "darkblue", main = "ROC Curve for the KNN model") # The "curve" looks like this because our model outputs discrete labels ("M" or"B") instead of continuous values
...
```
We now continue with a decision tree as model
```{r}
